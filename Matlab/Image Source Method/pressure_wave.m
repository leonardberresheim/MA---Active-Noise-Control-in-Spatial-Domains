...
% receiver for a given frequency at a time T.
%   out_p_abs = PRESSURE_WAVE(x0,x1,fe,mag,phase,t,c,Q,beta,spacing) 
%   Eingabe 
...
%       phase      signal phase
%       T          (1 x 1) single time point
%       c          speed of sound
%   Ausgabe 
%       out_p_abs  (m x n) output pressure for non-rigid walls
%
function out_p_abs = pressure_wave(x0,x1,fe,mag,phase, T, beta) 
    ...
    for j=1:size(x0,1)
        ...
        for i=1:length(t)
            ...
            if(t(i) < r/c)
                ...
                out_p(j,i) = mag*imag(1i*((Q*rho*c*k)/(4*pi*r))*exp(1i*(omega*t(i)-k*r+phase)));
