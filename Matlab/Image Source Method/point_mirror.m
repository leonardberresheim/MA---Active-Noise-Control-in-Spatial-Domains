...
% provided planes and their absorption coefficient.
...
function [M, post_beta] = point_mirror(plane, P, R, alpha, pre_beta) 
    ...
    % Delete the mirror points that are at the same position as the
    % receiver as this leads to computational problems
    post_beta = post_beta(~ismember(M,R,'rows'),:);
    M = M(~ismember(M,R,'rows'),:);
