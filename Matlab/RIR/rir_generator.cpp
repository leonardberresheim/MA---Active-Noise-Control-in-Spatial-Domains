...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    for (int idxMicrophone = 0; idxMicrophone < nMicrophones ; idxMicrophone++)
    {
        ...
        r[2] = rr[idxMicrophone + 2*nMicrophones] / cTs;
         std::string print_out = std::to_string(r[0]*cTs) + "\n";
         const char * c = print_out.c_str();
         mexPrintf(c);
        n1 = 1;//(int) ceil(nSamples/(2*L[0]));
        n2 = 1;//(int) ceil(nSamples/(2*L[1]));
        n3 = 1;//(int) ceil(nSamples/(2*L[2]));
        int zaehler = 0;
