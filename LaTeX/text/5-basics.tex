...
When multiplying both sides of \ref{eq:wave_field_decomposition} by $\overline Y_{um}$ and integrating over the sphere we get\cite{Chen2017}:\\
\begin{equation}
...
The spherical harmonic transform can be sampled using the Gaussian sampling scheme and is given by
\begin{equation}
    C_{lm}(k) = \frac{1}{j_l(kR)} \sum_{q=0}^N\sum_{p=0}^{2N+1}a_qP(R,\theta_q,\phi_p,k)\overline Y_{lm}(\theta_q,\phi_p), n \leq N.
\end{equation}
